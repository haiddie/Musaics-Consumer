...
import Albumart from "../../muzicard-prototype/components/AlbumArt";
import Listview from "../../muzicard-prototype/components/listview";
import MusicTrack from "../../muzicard-prototype/components/MusicTrack";
import MusicVideo from "../../muzicard-prototype/components/MusicVideo";
import Musicdownload from "../../muzicard-prototype/components/Download";
import BandsInTown from "../../muzicard-prototype/components/BandsInTown";
import { classNames } from "@/app/Util/styling";
import { useParams } from "next/navigation";
import Cookies from 'js-cookie';
import getSectionSVGComponent from "../../muzicard-prototype/components/getSVGComponent";
import { Squares2X2Icon } from "@heroicons/react/24/outline";
import AlbumartLarge from "../../muzicard-prototype/components/AlbumArtLarge";
import Bio from "../../muzicard-prototype/components/Bio";
import MailChimp from "../../muzicard-prototype/components/MailChimp";
import { MenuItem } from "@mui/material";
import StarIcon from '@mui/icons-material/Star';
import Extras from "../../muzicard-prototype/components/Extras";
...
const MuzicardDetails = ({ id }: Props) => {
  ...
  const getMuzicardData = async (device__id: string) => {
    setIsLoading(true)
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_HOST_URL}/api/consumer?muzicard_v3=true&ot_id=${id}&device_id=${device__id}`,
        { next: { revalidate: 3600 } }
      )
      const artistRes = await res.json();

      if (artistRes.data && artistRes.data.muzicard.length > 0) {
  ...
  const AddBookMark = () => {
    ...
    if (typeof window !== 'undefined') {
      let url = `${window.location.origin}/scanned-releases`
  ...
  return (
    <div className="relative font-cabin h-[100svh] w-screen">
      ...
      {/* <Image
            ...
            {hideGradient && (
              <>
                <div className={` transition-transform duration-1000 bg-black bg-opacity-50 p-2 left-[10%] sm:left-[43%] bottom-40 ${muzicardData?.muzicard[0]?.show_artist === false ? 'hidden' : 'absolute'}  ${showAnimation ? '-translate-y-[500px]' : '-translate-y-[300px]'}  !font-barrow text-[14px] z-20`}>{muzicardData?.muzicard[0]?.name}</div>
                <div className={` transition-transform duration-1000 bg-black bg-opacity-50 p-2 left-[10%] sm:left-[43%] bottom-20 ${muzicardData?.muzicard[0]?.show_title === false ? 'hidden' : 'absolute'} ${showAnimation ? '-translate-y-[500px]' : '-translate-y-[300px]'}  !font-barrow text-[32px] z-20`}>{muzicardData?.muzicard[0]?.title}</div>
              </>
            )
        ...
        <div className="flex flex-col min-h-[550px] justify-center items-center p-4">
          ...
          {muzicardData !== null ?
            <>
              ...
              {!hideGradient && muzicardData?.muzicard[0]?.show_title && <div className={`absolute text-center z-10 transition-transform duration-1000 ${showGradient ? 'transform-none' : 'translate-y-[100%]'} top-0 rounded-lg h-[100svh] w-screen bg-gradient-to-b from-transparent to-black`}>
                <div className={`absolute transition-transform duration-1000 left-1/2 -bottom-20 -translate-x-1/2 ${showAnimation ? '-translate-y-[500px]' : '-translate-y-[300px]'}  !font-abril text-7xl z-20`} dangerouslySetInnerHTML={{ __html: muzicardData?.muzicard[0]?.title }} />
              </div>}
              ...
              <div className="relative">
                  ...
                  <>
                    <div className={`h-[90svh] py-2 overflow-hidden`}>
                        ...
                        <div className={`${playState === false ? 'h-[90svh] overflow-y-hidden overflow-x-hidden' : 'h-[79svh] overflow-y-hidden overflow-x-hidden'} `}>
                          <div className={`h-[34svh]`}>
                            <div className="relative max-w-[250px] h-auto max-h-[32svh] w-full mx-auto">
                              ...
                              {
                                (muzicardData?.muzicard[0]?.show_artist || muzicardData?.muzicard[0]?.show_title) && (
                                  ...
                                  <div className="absolute bottom-0 left-0 right-0 bg-black bg-opacity-50 text-white px-2 py-2">
                                    <div className="flex flex-col justify-start items-center px-2">
                                      {
                                        muzicardData?.muzicard[0]?.show_artist && (
                                          <span className="!leading-[1.75rem] !text-[14px]" dangerouslySetInnerHTML={{ __html: muzicardData?.muzicard[0]?.artist_display_name }} />
                                        )
                                      }

                                      {
                                        muzicardData?.muzicard[0]?.how_title && (
                                          <span className="!leading-[2.75rem] !text-[32px]" dangerouslySetInnerHTML={{ __html: muzicardData?.muzicard[0]?.title }} />
                                        )}
                                    </div>
                                  </div>
                                )}
                          ...
                          <div className={`${playState === false ? 'h-[56svh]' : 'h-[47svh]'} mt-[20px] p-4  overflow-y-auto overflow-x-hidden`}>
                            {muzicardData?.muzicard[0]?.generic_details?.year_of_release && muzicardData?.muzicard[0]?.generic_details?.show_year_of_release ? (
                              <div className="text-white text-[14px] mb-1 !font-[400]">
                                Year of Release: {muzicardData?.muzicard[0]?.generic_details?.year_of_release}
                              </div>) : (<></>)}

                            {muzicardData?.muzicard[0]?.generic_details?.release_length && muzicardData?.muzicard[0]?.generic_details?.show_release_length ? (
                              <div className="text-white text-[14px] mb-1 !font-[400]">
                                Length of Release: {muzicardData?.muzicard[0]?.generic_details?.release_length ? convertToMmSs(muzicardData?.muzicard[0]?.generic_details?.release_length) : 0}
                              </div>) : (<></>)
                            }


                            {muzicardData?.muzicard[0]?.generic_details?.no_track && muzicardData?.muzicard[0]?.generic_details?.show_no_track ? (
                              <div className="text-white text-[14px] mb-1 !font-[400]">
                                Tracks: {sumUpTracks()}
                              </div>) : (<></>)
                            }

                            {muzicardData?.muzicard[0]?.generic_details?.no_downloads && muzicardData?.muzicard[0]?.generic_details?.show_no_downloads ? (
                              <div className="text-white text-[14px] mb-1 !font-[400]">
                                Downloads: {sumUpDownloads()}
                              </div>) : (<></>)
                            }
                            {muzicardData?.muzicard[0]?.description ?
                              (
                                <>
                                  <div
                                    className='text-white text-xl muzicard-bio'
                                    dangerouslySetInnerHTML={{ __html: transformContent(muzicardData?.muzicard[0]?.description) }}
                                  />
                                </>
                              )
                              : (<></>)
...

                      {
                        (activeTab === 'Extras') &&
                        <div className={`${playState === false ? 'h-[85svh] overflow-y-auto overflow-x-hidden' : 'h-[74svh] overflow-y-auto overflow-x-hidden'} `} >
                          {<Extras activeTab={activeTab} data={muzicardData.muzicard[0]} true_card={true} />}
                        </div>
                      }

                      {
                        ...
                        <div className={`${playState === false ? 'h-[85svh] overflow-y-auto overflow-x-hidden' : 'h-[74svh] overflow-y-auto overflow-x-hidden'} `} >
                          ...
                          {sectionDisplay?.mailchimp ? <MailChimp activeTab={activeTab} data={sectionDisplay?.mailchimp} default_image={muzicardData?.muzicard[0]?.image_url} /> : <div className='text-2xl'>
...

                      {
                        ...
                        <div className={`${activeTab === 'Music' ? 'h-[85svh] overflow-hidden' : 'h-[16svh] overflow-hidden'} `} >
                          {sectionDisplay?.music ? <MusicTrack activeTab={activeTab} downloads={sectionDisplay?.downloads} data={sectionDisplay?.music} updatePlayState={updatePlayState} default_image={muzicardData?.muzicard[0]?.image_url} /> : <div className='text-2xl'>
                            No Musics found!
                          </div>}
                        </div>
                      }
                ...
                {hideGradient && <div className={`text-sm z-50 font-bold w-screen h-[10svh] flex flex-nowrap items-center justify-around overflow-hidden bottom-0 py-2`}>
                  <div onClick={() => { setActiveTab('Home'); setActiveTabDisplayName('Home') }} className={`flex flex-col text-[10px] items-center gap-1 ${activeTab === 'Home' ? 'text-primary-100' : 'text-white'}`} style={{ fill: activeTab === 'Home' ? '#b247f5' : 'white' }}>
                                  ...
                                  {({ active }) => (
                                    <div onClick={() => { setActiveTab(section.name); setActiveTabDisplayName(section.section_display_name ? section.section_display_name : section.name) }}
                                      className={classNames(
                                        ...
                                        'px-2 py-2 text-[10px] flex items-center gap-1 cursor-pointer'
                                      )}>
                                      <div className="h-5 w-5 ">
                                        {getSectionSVGComponent(section.section_type, activeTab, '#64748B')}
                                      </div>
