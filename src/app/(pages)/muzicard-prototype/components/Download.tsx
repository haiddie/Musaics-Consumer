...
const Musicdownload = ({ data }: any) => {
    ...
    const [primary_tracks, setPrimaryTracks] = useState<any>([])
    const [bonus_tracks, setBonusTracks] = useState<any>([])
    ...
    useEffect(() => {
        if (data && data.length > 0) {
            ...
            let bonus: any[] = []
            data.map((track: any) => {
                if (track.main_content.type === 'primary') {
                    primary.push(track)
                }
                else {
                    bonus.push(track)
    ...
    const sumUpDownloadsDuration = (arr: any[]) => {
        let release_length = 0;
        arr.map((obj: any) => {
            if (
                obj.main_content &&
                ...
                release_length =
                    release_length + parseInt(obj.main_content.duration);
            }
        });
    ...
    return (
        <div>
            {!isDownloadLoading ? <div className="mt-5 w-full">
                {bonus_tracks && bonus_tracks?.length > 0 ?
                    <>
                        {
                            sumUpDownloadsDuration(bonus_tracks) > 0 &&
                            <div className="flex justify-between items-center w-full pt-[10px] gap-x-2"
                            >
                                <hr className="my-2 border-white basis-[30%] w-[30%] border-[2px]" />
                                <p className="!text-white text-md text-center basis-[40%] w-[40%] font-semibold">Bonus Tracks
                                </p>
                                <hr className="my-2 border-white basis-[30%] w-[30%] border-[2px]" />
                        ...
                        {
                            bonus_tracks?.map((item: any, index: number) => (
                                <div key={index}>
                                    {item?.main_content !== null && item?.status === 'published' && <div className="my-2">
                                        <h2 id={`accordion-arrow-icon-heading-${index + 1}`}>
                                         ...
                                         border-gray-700 text-white bg-gray-800  hover:bg-gray-800 gap-3`}
                                            >
                                                ...
                                                <div className={`text-[14px] break-all w-[70%] basis-[70%] font-medium rtl:text-right text-white capitalize`}
                                                    dangerouslySetInnerHTML={{
                                                        __html: (item?.main_content?.name.length
                                                            > 120
                                                            ?
                                                            item?.main_content?.name.slice(0, 120) + '...' :
                ...
                {primary_tracks && primary_tracks?.length > 0 ?
                    <>
                        {
                            sumUpDownloadsDuration(primary_tracks) > 0 &&
                            <div className="flex justify-between items-center w-full pt-[10px] gap-x-2"
                            >
                                <hr className="my-2 border-white basis-[30%] w-[30%] border-[2px]" />
                                <p className="!text-white text-md text-center basis-[40%] w-[40%] font-semibold">Primary Tracks
                                </p>
                                <hr className="my-2 border-white basis-[30%] w-[30%] border-[2px]" />
                        ...
                        {
                            primary_tracks?.map((item: any, index: number) => (
                                <div key={index}>
                                    {item?.main_content !== null && item?.status === 'published' && <div className="my-2">
                                        <h2 id={`accordion-arrow-icon-heading-${index + 1}`}>
                                            <button
