            ...
            {data?.main_content?.listarray.map((item: any, index: number) => (
                <a key={index} className="grid grid-cols-12 gap-2 my-2" href={item?.text ? `${item?.text}` : "#"} target='_blank'>
                    <div className="col-span-12">
                        <div className="flex flex-grow items-center justify-between gap-x-2 p-5 bg-[#0F172A] rounded-lg border-[0.3px] border-gray-500">
                            <div className="flex items-center justify-start w-[50px]"> {getIcon(item.icon_name)}</div>
