...
import SocialComment from "../social-comment/social-comment";
...
import InstagramEmbed from "../instagram-embed/image";
import "./parent.css"
...
const MusaicDetail = () => {
    ...
    const [playerSize, setPlayerSize] = useState({ width: '100%', height: '390' });
    const opts = {
    ...
    // #region STATES 
    const [musaicsData, setMusaicsData] = useState<any>();
    ...
    const getMusaicsData = async () => {
        try {
            const res = await fetch(
                `${process.env.NEXT_PUBLIC_HOST_URL}/api/consumer?article=true&slug=${params.id}`,
                { next: { revalidate: 3600 } }
            )
            const albumsRes = await res.json();

            setMusaicsData(albumsRes.data);
    ...
    const twitterShare = () => {
        handleTwitterShare(content_url, musaicsData?.content[0]?.title, musaicsData?.content[0]?.image_description, isDesktop, musaicsData?.content[0]?.image);
    };
    ...
    const [imageContainerWidth, setimageContainerWidth] = useState<any>(280)
    ...
    return (
        <div className='bg-black  lg:mt-24 xl:mt-24 text-white font-poppins'>
            <div className={classNames('pt-6')}>
                <div className={classNames('overflow-x-hidden', 'mx-auto', 'max-w-7xl', 'px-1', 'sm:px-6', 'lg:px-8',)}>
                    <div className={classNames('lg:grid', 'lg:auto-rows-min', 'lg:grid-cols-12', 'lg:gap-x-16')}>


                        {/* Image gallery */}
                        <div className={classNames('mt-8', 'lg:col-span-6', 'lg:col-start-1', 'lg:row-span-3', 'lg:row-start-1', 'lg:mt-0')}>
                            <h2 className={classNames('sr-only')}>Images</h2>

                            <div className={classNames('grid', 'grid-cols-1', 'grid-cols-1')}>

                                {musaicsData ? (
                                    <div className={`relative my-6 mx-auto w-full max-w-full min-w-[${imageContainerWidth}px] sm:min-w-[440px] xl:min-w-[500px] sm:max-w-[440px] xl:max-w-[500px]`}>
                                        {musaicsData?.content[0]?.thumbnail ? <Image
                                            src={musaicsData?.content[0]?.thumbnail}
                                            alt="Tappi"
                                            width={650}
                                            height={350}

                                            className={`mx-auto max-w-full sm:max-w-[440px] xl:max-w-[500px] rounded-md object-cover`}
                                        /> : <div style={{ height: imageContainerHeight + 'px' }} className={`mx-auto max-w-full sm:max-w-[440px] xl:max-w-[500px]  bg-gray-400 rounded-md text-8xl text-gray-700 items-center flex justify-center`}>
                                            {getInitials(musaicsData?.content[0]?.title)}
                                        </div>}

                                        <div className={` absolute top-0  max-w-full sm:max-w-[500px] xl:max-w-[500px] left-0 z-5 h-full w-full bg-gradient-to-b from-transparent to-black rounded-lg`}>
                                        </div>

                                        {musaicsData && musaicsData?.content[0]?.title ?
                                            <div className={`absolute bottom-0 max-w-full xs:max-w-[500px] lg:hidden w-full z-10 overflow-hidden !p-[25px]`}>

                                                <div className='flex flex-col  gap-y-1 mt-2'>
                                                    <span className='text-violet-500 text-sm font-bold mb-2'>{new Date(musaicsData?.content[0]?.published_date).toLocaleDateString('en-US', {
                                                        year: 'numeric',
                                                        month: 'short',
                                                        day: '2-digit',
                                                    })}</span>
                                                </div>
                                                <div dangerouslySetInnerHTML={{ __html: musaicsData?.content[0]?.title }} className='text-[1.5rem] leading-[2rem] font-semibold line-clamp-3 text-white mb-2'></div>
                                                {
                                                    musaicsData?.content[0]?.organization ? <div dangerouslySetInnerHTML={{ __html: musaicsData?.content[0]?.organization }} className='text-base font-semibold line-clamp-3 text-white text-right' /> : <></>
                                                }
                                            </div> :
                                    ...
                                    <div className="h-[834px] w-[full] xs:w-[500px] mb-6 sm:mb-0  flex items-center justify-center rounded-lg bg-gray-700">
                                ...
                                <div className="hidden lg:flex justify-end w-[full] xs:max-w-[500px] xl:max-w-[500px]">
                                    {musaicsData?.content[0]?.image_credit ? <a className="text-gray-500 text-[12px] lg:text-[12px] xl:text-[12px]  pr-0 lg:pr-0 xl:pr-0 text-right"
                                        href={musaicsData?.content[0]?.image}>
                                        Source: <span className="text-violet-500">
                                            &#xA9; {musaicsData?.content[0]?.image_credit}
                                        </span>
                                    </a> : <></>}
                                </div>


                                <div className="mt-5 hidden lg:block w-[full] xs:max-w-[500px] xl:max-w-[500px] no-mobile">

                                    <div className="h-[1px] w-full bg-[#daddca]"></div>
                                    <div className="py-2 font-RohnRounded-Medium text-grey text-center border-b border-b-[#daddca]">
                                        <span className="!text-white">   SHARE</span>
                                    </div>

                                    <div className="flex items-center flex-col gap-y-1">
                                        <div className="w-full flex justify-center py-1 items-center border-b-[#daddca] border-b-[1px]">
                                            ...
                                            <svg onClick={() => twitterShare()} version="1.1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 600 600" width="80" height="40" className=" cursor-pointer">
                                        ...
                                        <div className="w-full flex justify-center items-center py-1 border-b-[#daddca] border-b-[1px]">
                        ...
                        <div className={'col-span-12 lg:col-span-6'}>
                            {musaicsData && musaicsData?.content[0]?.post ? <div className="mx-auto lg:mx-4 xs:max-w-[500px] md:max-w-[600px] lg:max-w-full ">

                                <div dangerouslySetInnerHTML={{ __html: musaicsData?.content[0]?.title }} className='hidden lg:block text-4xl leading-[3rem] font-bold line-clamp-3 text-violet-500 mb-2'></div>
                                {musaicsData?.content[0]?.image_credit ?
                                    <div className="flex justify-end w-full px-4">
                                        <a className="text-gray-500 lg:hidden text-[12px]  lg:text-[12px] xl:text-[12px]  pr-0 lg:pr-0 xl:pr-0 xl:text-right"
                                            href={musaicsData?.content[0]?.image}>
                                            Source: <span className="text-violet-500">
                                                &#xA9; {musaicsData?.content[0]?.image_credit}
                                            </span>
                                    ...
                                    : <></>}
                                <div >
                                    <div className="text-grey text-lg mb-1 font-RohnRounded-Bold px-4 lg:px-0">
                                        {new Date(musaicsData?.content[0]?.published_date).toLocaleDateString('en-US', {
                                            year: 'numeric',
                                            month: 'short',
                                            day: '2-digit',
                                        })}
                                    </div>
                                    <div className="flex lg:hidden items-center gap-x-4 px-4 lg:px-0">
                                        {musaicsData?.content[0]?.authors !== null ?
                                            <div>
                                                {
                                                    musaicsData?.content[0]?.author_slug ?
                                                        // <a
                                                        //     className="font-RohnRounded-Medium text-lg text-grey"
                                                        //     href={`/writer/${musaicsData?.content[0]?.author_slug}`}>
                                                        //     {musaicsData?.content[0]?.authors}
                                                        // </a> :
                                                        <span
                                                            className="font-RohnRounded-Medium text-lg text-grey">
                                                            {musaicsData?.content[0]?.authors}
                                    ...
                                    <div className="flex lg:hidden items-center gap-x-4 px-4 lg:px-0">
                                        {
                                            musaicsData?.content[0]?.source !== null ?
                                                <div className="font-RohnRounded-Medium text-lg text-grey"
                                                >
                                                    {musaicsData?.content[0]?.source}
                                                </div> : <div className="font-RohnRounded-Medium text-lg text-grey"
                                                >
                                                    TAPPI
                                                </div>
                                ...
                                <div className="mt-5 lg:hidden px-4">
                                ...
                                <div
                                    ...
                                    className={`px-4 lg:px-0 w-full overflow-hidden transition-[height] duration-300`}
                                // style={{ maxHeight: isCollapsed ? '40em' : 'none', WebkitLineClamp: isCollapsed ? 20 : 'none' }}
                                >
                                    <div
                                        className={classNames('prose', 'prose-lg', 'mt-4', 'text-gray-400', 'text-md')}
                                        dangerouslySetInnerHTML={{ __html: musaicsData?.content[0]?.post }}
                                    />
                                </div>
                                {/* <p className="mt-2">
                                    <button
                                    ...
                                    >
                                        <span className="text-xl cursor-pointer">{isCollapsed ? 'Read more' : 'Read less'}</span>
                                        <svg
                                            className={`hs-collapse-open:rotate-180 flex-shrink-0 w-4 h-4 transition-transform ${isCollapsed ? 'transform rotate-0' : 'transform rotate-180'
                                                }`}
                    ...
                    <div className="flex w-full justify-center pt-14 mb-8 px-2">
                        <div className="flex flex-col gap-x-8 justify-center sm:w-[99%] w-full xs:max-w-[500px] md:max-w-[600px]">
                            {musaicsData?.blocks?.length > 0 &&
                                musaicsData?.blocks?.map((element: any, index: number) => (
                                    <div key={index}>{
                                        element.status === 'published' && <div key={index} id={`${index}`}>
                                            {element?.type === 'twitter' && (
                                                <>
                                                    <div className="flex flex-col items-center gap-y-2 justify-center border-dashed border-2 bg-[#0F172A] border-primary-500 rounded-2xl px-2 sm:px-5 py-5">
                                                        {element?.is_title && element.title && element.title !== "null" && (<span className='text-white text-center capitalize text-xl font-bold  px-[10px] py-[10px]' style={{ overflowWrap: 'anywhere' }} dangerouslySetInnerHTML={{ __html: element.title }} />)}
                                                    ...
                                                    {
                                                        index !== musaicsData?.blocks?.length - 1 &&
                                            ...
                                            {element?.type === 'video' && (
                                                <>
                                                    <div className="flex flex-col items-center justify-center border-dashed border-2 gap-y-2 bg-[#0F172A] border-primary-500 rounded-2xl px-2 sm:px-5 py-5">
                                                        {element?.is_title && element.title && element.title !== "null" && (<span className='text-white text-center capitalize text-xl font-bold  px-[10px] py-[10px]' style={{ overflowWrap: 'anywhere' }} dangerouslySetInnerHTML={{ __html: element.title }} />)}
...

                                            {
                                                ...
                                                element?.type === 'polls' &&
                                                <>
                                                    <div className="flex flex-col items-center justify-center w-full border-dashed gap-y-2 border-2 bg-[#0F172A] border-primary-500 rounded-2xl px-2 sm:px-5 py-5">
...

                                            {
                                                ...
                                                element?.type === 'instagram_embed' &&
                                                <>
                                                    <div className="flex flex-col items-center justify-center w-full border-dashed gap-y-2 border-2 bg-[#0F172A] border-primary-500 rounded-2xl px-2 sm:px-5 py-5 ">

                                                        <InstagramEmbed data={element} />
                                                    </div>
                                                    {
                                                        index !== musaicsData?.blocks?.length - 1 &&
                                                        <div className="h-8 w-[1px] mx-auto border-l-2 border-dashed border-violet-500"></div>
...

                                            {
                                                ...
                                                element?.type === 'bill_board' &&
                                                <>
                                                    <div className="flex justify-center w-full border-dashed gap-y-2 border-2 bg-[#0F172A] border-primary-500 rounded-2xl  px-0 sm:px-4  py-4">

                                                        <Billboard widget={element} />
...

                                            {
                                                ...
                                                element?.type === 'artist_card' &&
                                                <>
                                                    <div className="flex flex-col items-center justify-center w-full border-dashed gap-y-2 border-2 bg-[#0F172A] border-primary-500 rounded-2xl  px-0 sm:px-4  py-4">
                                                        {element?.is_title && element.title && element.title !== "null" && (<span className='text-white text-center capitalize text-xl font-bold   px-[10px] py-[10px]' style={{ overflowWrap: 'anywhere' }} dangerouslySetInnerHTML={{ __html: element.title }} />)}
...

                                            {
                                                ...
                                                element?.type === 'social_comment' &&
                                                <>
                                                    <div className="flex justify-center w-full border-dashed gap-y-2 border-2 bg-[#0F172A] border-primary-500 rounded-2xl  px-2 sm:px-5 py-5">

                                                        <SocialComment data={element} />
                                                    </div>
                                                    {
                                                        index !== musaicsData?.blocks?.length - 1 &&
                                                        <div className="h-8 w-[1px] mx-auto border-l-2 border-dashed border-violet-500"></div>
...

                                            {
                                                ...
                                                element?.type === 'chart' &&
                                                <>
                                                    <div className="flex flex-col items-center justify-center w-full border-dashed gap-y-2 border-2 border-primary-500 bg-[#0F172A] rounded-2xl  px-2 sm:px-5 py-5">
...

                                            {
                                                ...
                                                element?.type === 'music_track' &&
                                                <>
                                                    <div className="flex flex-col justify-center sm:justify-start items-center gap-y-2 w-full border-dashed border-2 bg-[#0F172A]  border-primary-500 rounded-2xl   px-2 sm:px-5 py-5">
                                                        {element?.is_title && element.title && element.title !== "null" && (<span className='text-white text-center capitalize text-xl font-bold  px-[10px] py-[10px]' style={{ overflowWrap: 'anywhere' }} dangerouslySetInnerHTML={{ __html: element.title }} />)}
                                                        <MuxPlayer
                                                            style={{ maxWidth: isDesktop ? '600px' : '450px', padding: isDesktop ? '20px' : '4px', height: '80px', width: isDesktop ? '80vw' : '90vw' }}
                                                            audio
                                                            playbackId={element?.main_content.playback_id}
                                                            metadata-video-title={element?.title}
                                                            theme="Classic"
...

                                                        {
                                                            element.is_text &&
                                                            <div
                                                                className={`text-md text-base text-left`}
                                                                dangerouslySetInnerHTML={{ __html: element?.display_text }}
                                                            />
                                                        }
                                                    </div>
